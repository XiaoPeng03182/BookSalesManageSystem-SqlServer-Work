...
import com.example.booksalesmanagement.databinding.ActivityDeleteCartDetailsBinding
import kotlin.concurrent.thread

class DeleteCartDetailsActivity : AppCompatActivity() {
    private var _binding: ActivityDeleteCartDetailsBinding? = null
    private val binding get() = _binding!!

    private var cartList = ArrayList<CartDetails>()

    private var adapter: DeleteCartDetailsAdapter? = null

    @SuppressLint("NotifyDataSetChanged", "ResourceAsColor")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        _binding = ActivityDeleteCartDetailsBinding.inflate(layoutInflater)
        setContentView(binding.root)

        //初始化适配器
        val layoutManager = LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false)
        binding.recyclerView.layoutManager = layoutManager
        adapter = DeleteCartDetailsAdapter(this, cartList)
        binding.recyclerView.adapter = adapter

        //查询按钮监听
        binding.btnQueryCartByName.setOnClickListener {
            //binding.SearchBookLineLayout.visibility = View.GONE
            val inputUserName = binding.etQueryCartByName
                .text.toString()
            thread {
    ...
    private fun refreshCart(adapter: DeleteCartDetailsAdapter) {

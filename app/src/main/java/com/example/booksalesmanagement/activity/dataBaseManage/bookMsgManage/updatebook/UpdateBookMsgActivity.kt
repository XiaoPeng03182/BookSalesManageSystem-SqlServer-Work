...
import com.example.booksalesmanagement.databinding.ActivityUpdateBookMsgBinding
import kotlin.concurrent.thread

class UpdateBookMsgActivity : AppCompatActivity() {
    private var _binding: ActivityUpdateBookMsgBinding? = null
    private val binding get() = _binding!!

    private var bookList = ArrayList<Book>()

    private var adapter: UpdateBookAdapter? =null

    @SuppressLint("NotifyDataSetChanged", "ResourceAsColor")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        _binding = ActivityUpdateBookMsgBinding.inflate(layoutInflater)
        setContentView(binding.root)

        //初始化适配器
        val layoutManager = LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false)
        binding.recyclerView.layoutManager = layoutManager
        adapter = UpdateBookAdapter(this, bookList)
    ...
    private fun refreshBooks(adapter: UpdateBookAdapter) {
        thread {

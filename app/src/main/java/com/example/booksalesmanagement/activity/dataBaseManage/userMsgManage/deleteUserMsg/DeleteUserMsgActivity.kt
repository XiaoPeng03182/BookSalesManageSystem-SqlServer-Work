...
import com.example.booksalesmanagement.activity.dataBaseManage.userMsgManage.queryUserMsg.QueryUserMsgAdapter
...
import com.example.booksalesmanagement.databinding.ActivityDeleteUserMsgBinding
import kotlin.concurrent.thread

class DeleteUserMsgActivity : AppCompatActivity() {
    private var _binding: ActivityDeleteUserMsgBinding? = null
    private val binding get() = _binding!!

    private var userList = ArrayList<User>()

    private var adapter: DeleteUserMsgAdapter? = null

    @SuppressLint("NotifyDataSetChanged", "ResourceAsColor")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        _binding = ActivityDeleteUserMsgBinding.inflate(layoutInflater)
        setContentView(binding.root)

        //初始化适配器
        val layoutManager = LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false)
        binding.recyclerView.layoutManager = layoutManager
        adapter = DeleteUserMsgAdapter(this, userList)
    ...
    private fun refreshCart(adapter: DeleteUserMsgAdapter) {
        thread {

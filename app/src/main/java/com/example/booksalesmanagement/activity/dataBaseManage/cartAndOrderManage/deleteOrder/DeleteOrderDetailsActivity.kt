...
import com.example.booksalesmanagement.databinding.ActivityDeleteOrderDetailsBinding
import kotlin.concurrent.thread

class DeleteOrderDetailsActivity : AppCompatActivity() {
    private var _binding: ActivityDeleteOrderDetailsBinding? = null
    private val binding get() = _binding!!

    private var cartList = ArrayList<OrderDetails>()

    private var adapter: DeleteOrderDetailsAdapter? = null

    @SuppressLint("NotifyDataSetChanged", "ResourceAsColor")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        _binding = ActivityDeleteOrderDetailsBinding.inflate(layoutInflater)
        setContentView(binding.root)

        //初始化适配器
        val layoutManager = LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false)
        binding.recyclerView.layoutManager = layoutManager
        adapter = DeleteOrderDetailsAdapter(this, cartList)
    ...
    private fun refreshCart(adapter: DeleteOrderDetailsAdapter) {
        val inputUserName = binding.etQueryOrderByName.text.toString()

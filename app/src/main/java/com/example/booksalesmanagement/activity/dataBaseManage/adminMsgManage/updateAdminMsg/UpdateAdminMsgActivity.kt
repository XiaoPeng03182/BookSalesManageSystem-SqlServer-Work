...
import com.example.booksalesmanagement.activity.dataBaseManage.adminMsgManage.deleteAdminMsg.DeleteAdminMsgAdapter
import com.example.booksalesmanagement.bean.Administrator
import com.example.booksalesmanagement.dao.AdminDao
import com.example.booksalesmanagement.databinding.ActivityUpdateAdminMsgBinding
import kotlin.concurrent.thread

class UpdateAdminMsgActivity : AppCompatActivity() {
    private var _binding: ActivityUpdateAdminMsgBinding? = null
    private val binding get() = _binding!!

    private var adminList = ArrayList<Administrator>()

    private var adapter: UpdateAdminMsgAdapter? = null


    @SuppressLint("NotifyDataSetChanged", "ResourceAsColor")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        _binding = ActivityUpdateAdminMsgBinding.inflate(layoutInflater)
        setContentView(binding.root)

        //初始化适配器
        val layoutManager = LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false)
        binding.recyclerView.layoutManager = layoutManager
        adapter = UpdateAdminMsgAdapter(this, adminList)
        binding.recyclerView.adapter = adapter

        //查询按钮监听
        binding.btnUpdateAdminMsgByName.setOnClickListener {
            val inputAdminName = binding.etUpdateAdminMsgByName.text.toString()
            thread {
    ...
    private fun refreshAdmin(adapter: UpdateAdminMsgAdapter) {
        thread {

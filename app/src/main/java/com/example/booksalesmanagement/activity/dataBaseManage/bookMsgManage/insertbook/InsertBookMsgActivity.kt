package com.example.booksalesmanagement.activity.dataBaseManage.bookMsgManage.insertbook
...
class InsertBookMsgActivity : AppCompatActivity() {
    ...
    private fun commitInsertBook() {
        ...
        if (binding.etBookId.text.isEmpty() || binding.etBookName.text.isEmpty()
            || binding.etBookAuthor.text.isEmpty() || binding.etBookPrice.text.isEmpty()
            || binding.etPublisher.text.isEmpty() || binding.etPublisherTime.text.isEmpty()
            || binding.etInventory.text.isEmpty() || binding.etBookInfo.text!!.isEmpty()
            || binding.etIsbn.text.isEmpty() || binding.etBookCategory.text.isEmpty()
        ) {
            Toast.makeText(this,"请输入图书信息！",Toast.LENGTH_SHORT).show()
        }

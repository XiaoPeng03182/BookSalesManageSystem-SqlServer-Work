...
import com.example.booksalesmanagement.activity.dataBaseManage.adminMsgManage.queryAdminMsg.QueryAdminMsgAdapter
import com.example.booksalesmanagement.bean.Administrator
import com.example.booksalesmanagement.dao.AdminDao
import com.example.booksalesmanagement.databinding.ActivityDeleteAdminMsgBinding
import kotlin.concurrent.thread

class DeleteAdminMsgActivity : AppCompatActivity() {
    private var _binding: ActivityDeleteAdminMsgBinding? = null
    private val binding get() = _binding!!

    private var adminList = ArrayList<Administrator>()

    private var adapter: DeleteAdminMsgAdapter? = null

    @SuppressLint("NotifyDataSetChanged", "ResourceAsColor")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        _binding = ActivityDeleteAdminMsgBinding.inflate(layoutInflater)
        setContentView(binding.root)

        //初始化适配器
        val layoutManager = LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false)
        binding.recyclerView.layoutManager = layoutManager
        adapter = DeleteAdminMsgAdapter(this, adminList)
        binding.recyclerView.adapter = adapter

        //查询按钮监听
        binding.btnDeleteAdminMsgByName.setOnClickListener {
            val inputAdminName = binding.etDeleteAdminMsgByName.text.toString()
            thread {
    ...
    private fun refreshAdmin(adapter: DeleteAdminMsgAdapter) {
        thread {
